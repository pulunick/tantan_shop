...
**회원가입 즉시 로그인 적용**(2026-07-13): dev Supabase "Confirm email" OFF 완료(대시보드). 가입 성공 시 세션 포함 → 자동 로그인+홈 이동, 미포함 시 로그인 안내 폴백. 사용자 테스트 후 발견 버그 2건 수정 완료(미인증 로그인 불가 원인 규명, 탈퇴 다이얼로그 중앙 정렬).

**수동 테스트 체크포인트**(진행 중):

- [ ] 가입 즉시 로그인 + 비회원 장바구니 병합 + 중복 이메일 에러 (인증 OFF 상태에서)
- [ ] UX 보강 배치(주문/상세/상품/문의조회/관리자) 체크리스트
